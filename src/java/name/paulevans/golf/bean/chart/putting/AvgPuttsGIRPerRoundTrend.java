...
public class AvgPuttsGIRPerRoundTrend extends ChartAdapter {
	...
	public JFreeChart getChart(Date aFromDate, Date aToDate, 
		...
		series = new TimeSeries(getLabelStringFactory().getString(
				"chart.totalputtsgir.seriesname"));
		...
		for (ScorecardSummaryDAO score : scores) {
			if (score.getTotalPuttsGir() != null) {
				series.add(new Day(score.getDatePlayed()), new Double(
						score.getTotalPuttsGir()));
			}
		}		
		return ChartUtils.newTimeSeriesChart(series, 
				getLabelStringFactory().getString("chart.totalputtsgir.title"), 
				getLabelStringFactory().getString("chart.totalputtsgir.title.yaxis"), null,
				false);
	}
}
