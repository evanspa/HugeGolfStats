...
public class LongDriveDistanceTrend extends ChartAdapter {
	...
	public JFreeChart getChart(Date aFromDate, Date aToDate, 
		...
		series = new TimeSeries(getLabelStringFactory().getString(
				"chart.avglongestdrivedistance.seriesname"));
		...
		for (ScorecardSummaryDAO score : scores) {
			if (score.getLongestDrive() != null) {
				series.add(new Day(score.getDatePlayed()), new Double(
						score.getLongestDrive()));
			}
		}		
		return ChartUtils.newTimeSeriesChart(series, 
				getLabelStringFactory().getString("chart.avglongestdrivedistance.title"), 
				getLabelStringFactory().getString(
						"chart.avglongestdrivedistance.title.yaxis"), 
				null, false);
	}
}
