...
public class APSDistanceBreakdownPie extends ChartAdapter {
	...
	public JFreeChart getChart(Date aFromDate, Date aToDate, 
		...
		String[] APS_DISTANCE_CATEGORIES = {
			getLabelStringFactory().getString(
				"chart.aps.distance.breakdownpiechart.pinhigh"),
			getLabelStringFactory().getString(
				"chart.aps.distance.breakdownpiechart.notpinhigh")
		};
		...
		dataset = new DefaultPieDataset();
		dataset.setValue(APS_DISTANCE_CATEGORIES[0], apsLineIds[1]);
		dataset.setValue(APS_DISTANCE_CATEGORIES[1], apsLineIds[0] + 
				apsLineIds[2]);	
		chart = ChartFactory.createPieChart3D(getLabelStringFactory().getString(
				"chart.aps.distance.breakdownpiechart.title"), dataset, true, false, 
				false);
