...
<script language="JavaScript">
	...
	function newSearch() {
		...
		formObj.action="EditRound-search.do";
		formObj.submit();
	}
	...
	function nextResultPage() {
		var formObj = document.getElementById("mainForm");
		formObj.action="EditRound-search-results.do";
...
<h2><bean:message key="round.edit.searchresults.header" /></h2>

<html:form styleId="mainForm" focus="courseName" action="/secure/EditRound-page1">
	
	<html:hidden property="firstSearchResultNum" />
	<html:hidden property="pageAction" value="<%= Constants.TRUE %>" />
			
	<h3><bean:message key="round.edit.searchresults.pickroundtoedit" /></h3>
	<jsp:include page="include-searchresultspaginglinks.jsp" />
	<table class="list-results">
		<tr>
			<th style="text-align:center">&nbsp;</th>
			<th style="text-align:center"><a href="EditRound-search-results-sort.do?<%= WebConstants.SORT_COLUMN_PARAM %>=<%= WebConstants.ROUNDS_COURSE_COL %>"><bean:message key="search.coursename" /></a></th>
			<th style="text-align:center"><a href="EditRound-search-results-sort.do?<%= WebConstants.SORT_COLUMN_PARAM %>=<%= WebConstants.ROUNDS_DATEPLAYED_COL %>"><bean:message key="search.round.dateplayed" /></a></th>
			<th style="text-align:center"><a href="EditRound-search-results-sort.do?<%= WebConstants.SORT_COLUMN_PARAM %>=<%= WebConstants.ROUNDS_NUMHOLESPLAYED_COL %>"><bean:message key="search.round.numholesplayed" /></a></th>
			<th style="text-align:center"><a href="EditRound-search-results-sort.do?<%= WebConstants.SORT_COLUMN_PARAM %>=<%= WebConstants.ROUNDS_SCORE_COL %>"><bean:message key="search.round.score" /></a></th>
			<th style="text-align:center"><a href="EditRound-search-results-sort.do?<%= WebConstants.SORT_COLUMN_PARAM %>=<%= WebConstants.ROUNDS_ISTOURNEY_COL %>"><bean:message key="search.round.istournament" /></a></th>
			<th style="text-align:center"><a href="EditRound-search-results-sort.do?<%= WebConstants.SORT_COLUMN_PARAM %>=<%= WebConstants.ROUNDS_CITY_COL %>"><bean:message key="search.city" /></a></th>
		</tr>
		<%
		...
		<logic:iterate indexId="i" id="round" name="<%= AttributeKeyConstants.ROUNDS %>">
			...
			%>
			<tr>
				<td style="text-align: center"><span style="font-size:larger">[</span><a href="EditRound-page1.do?scorecardid=${round.scorecard.id}"><bean:message key="search.round.edit" /></a><span style="font-size:larger">]</span></td>
